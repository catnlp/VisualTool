...
def _get_html_text(text, gold_list, pred_list):
    ...
    while gold_idx < len(gold_list) and \
        ...
        if gold_start == pred_start and \
            ...
            gold_idx += 1
            pred_idx += 1
        elif gold_start < pred_start and \
                gold_end < pred_end and \
                gold_end > pred_start:
...
def upload(request):
    ...
    if gold_file:
        ...
        for key in gold_label:
            ...
            gold_pie_series_data.append(
                {"name": key, "value": gold_label[key]})
            if key in pred_label:
                bar_dataset_source.append(
                    [key, gold_label[key], pred_label[key]])
            else:
    ...
    if pred_file:
        ...
        for key in pred_label:
            ...
            pred_pie_series_data.append(
                {"name": key, "value": pred_label[key]})
